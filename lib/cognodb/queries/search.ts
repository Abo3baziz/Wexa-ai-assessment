...
export const SEARCH_QUERY = `
  CALL {
    ...
    RETURN 'Disease' AS type, dis.id AS id, dis.name AS label, dis.category AS subtitle
    UNION
    MATCH (m:Medication) WHERE toLower(m.name) CONTAINS toLower($q)
    RETURN 'Medication' AS type, m.id AS id, m.name AS label, m.dosageForm AS subtitle
  }
  RETURN type, id, label, subtitle
  ORDER BY type, label
  LIMIT $limit
`;
